...
export const AgentDashboard: React.FC<AgentDashboardProps> = ({ currentUser, serverUrl, onLogout, onNavigate }) => {
  ...
  const fetchAgents = async () => {
    try {
      const response = await fetch(`${API_URL}/api/membros`, { mode: 'cors' });
      if (response.ok) {
    ...
    <div className="flex flex-col h-full bg-[#111827] text-slate-100 overflow-hidden font-sans">
      ...
      <div className="h-16 bg-[#1f2937] border-b border-gray-700 flex items-center justify-between px-6 shrink-0">
         ...
         <div className="flex items-center gap-6 text-sm">
             ...
             <div className="flex items-center gap-4 border-l border-gray-600 pl-6">
                 <button className="flex items-center gap-2 text-yellow-500 font-medium hover:text-yellow-400 transition-colors">
      ...
      <div className="flex-1 overflow-y-auto p-6 bg-[#111827]">
          ...
          <div className="flex flex-col md:flex-row items-center justify-between mb-6 gap-4">
              ...
              <div className="flex items-center gap-3">
                  <button className="px-4 py-2 bg-gray-600 hover:bg-gray-500 text-white rounded-lg text-sm font-medium flex items-center gap-2">
                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 18h.01M8 21h8a2 2 0 002-2V5a2 2 0 00-2-2H8a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                     WhatsApp (0)
                  </button>
                   <button className="px-4 py-2 bg-green-700 hover:bg-green-600 text-white rounded-lg text-sm font-medium flex items-center gap-2">
                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>
                     CSV
                  </button>
                   <button className="px-4 py-2 bg-red-700 hover:bg-red-600 text-white rounded-lg text-sm font-medium flex items-center gap-2">
                     <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z" /></svg>
                     PDF
                  </button>
                  <button 
                    onClick={() => onNavigate(ViewState.REGISTER)}
                    className="px-6 py-2 bg-blue-600 hover:bg-blue-500 text-white rounded-lg text-sm font-bold flex items-center gap-2 shadow-lg shadow-blue-900/20"
                  >
          ...
          {/* Select All Bar */}
          <div className="bg-[#1f2937] p-3 rounded-lg mb-6 flex items-center gap-3 border border-gray-700">
              <div className="w-4 h-4 border border-gray-500 rounded bg-white"></div>
              <span className="text-sm text-gray-300 font-medium">Selecionar todos (0 / {filteredAgents.length})</span>
          </div>
                ...
                {filteredAgents.map(agent => (
                    <div key={agent.id} className="bg-[#1f2937] border border-gray-700 rounded-xl p-6 relative group hover:border-gray-500 transition-all">
                        ...
                        <div className="space-y-2 text-sm text-gray-400">
                            ...
                            <div className="flex items-center gap-2">
                                <svg className="w-4 h-4 opacity-70" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" /></svg>
                                {agent.estado_civil}
                            </div>
                        </div>

                        <div className="absolute bottom-4 right-4 flex gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                            <button className="text-gray-400 hover:text-white" title="Editar">
                                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" /></svg>
                            </button>
                            <button className="text-gray-400 hover:text-red-400" title="Excluir">
