      ...
      try {
          ...
          if (response.ok) {
              if (text.trim().startsWith('<!DOCTYPE html>') || text.includes('ngrok-free.dev')) {
                  ...
                  alert("Conexão parcial! O Ngrok exibiu uma tela de aviso.\nAtualize o 'server.js' (Aba 3) para corrigir.");
              } else {
           ...
           if (err.includes('aborted')) {
               addLog('error', 'Timeout', `O servidor não respondeu em 8s. Verifique IP e Firewall.`);
           } else if (err.includes('Failed to fetch')) {
               addLog('error', 'Falha de Rede/CORS', `Não foi possível conectar. Verifique se o server.js permite CORS e se o endereço está correto.`);
           } else {
               addLog('error', 'Erro de Conexão', err);
           }
      } finally {
          setTesting(false);
      }
...
CREATE TABLE IF NOT EXISTS eventos (
  id BIGINT GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY,
  titulo TEXT DEFAULT 'Missa da Saúde',
  descricao TEXT,
...
pool.connect((err) => {
  if (err) console.error('❌ Erro Fatal DB:', err.message);
...
app.post('/api/eventos/:id/duplicate', async (req, res) => {
  try {
    ...
    if (!new_date) throw new Error('Nova data é obrigatória');
      ...
      <div className="bg-blue-900/30 border border-blue-500/30 rounded-xl p-4 mb-6 flex flex-col gap-4">
        ...
        {showMixedContentWarning && (
            <div className="bg-orange-500/20 border border-orange-500/50 p-3 rounded text-orange-200 text-xs flex items-start gap-2">
                ...
                <div>
                    <strong>Atenção (Mixed Content):</strong> O navegador pode bloquear conexões HTTPS para HTTP local.
                    <br/>Solução: Use o Ngrok (aba 4) para criar um túnel seguro (https).
                </div>
            </div>
        )}
                ...
                <div className="space-y-8 text-sm text-slate-300">
                    ...
                    <div className="bg-red-900/10 border border-red-500/20 p-4 rounded-lg">
                        <h3 className="text-red-300 font-bold text-lg mb-2 flex items-center gap-2">
                           ⚠️ IMPORTANTE: Atualização para Módulo Idosos
                        </h3>
                        <p className="mb-2">Para habilitar o módulo "Missa da Saúde", você precisa rodar os scripts SQL e atualizar o Node.js.</p>
                        <ol className="list-decimal pl-5 space-y-2 text-white">
                            <li>Vá para a aba <strong className="text-blue-300">2. SQL Tabela</strong> e rode o script no seu PGAdmin/DBeaver.</li>
                            <li>Vá para a aba <strong className="text-green-300">3. Node.js API</strong>, copie o código e substitua no seu arquivo <code className="text-green-300">server.js</code>.</li>
                    ...
                    <div className="space-y-3">
                        ...
                        <p>Copie o <strong>IPv4</strong> e cole no campo acima (se não for usar Ngrok).</p>
                    </div>
                </div>
            )}
            ...
            {activeTab === 'ngrok' && (
                <div className="space-y-6 text-sm text-slate-300">
                     <div className="bg-orange-900/20 border border-orange-500/20 p-4 rounded-lg">
                        <h3 className="text-orange-300 font-bold text-lg mb-2">Solução Definitiva (Ngrok)</h3>
                        <p>Use o Ngrok para criar um endereço HTTPS seguro para seu servidor local. Isso resolve erros de "Mixed Content" e bloqueios do Vercel.</p>
                    </div>
                    ...
                    <ol className="list-decimal pl-5 space-y-4">
                        ...
                        <li>
                            No terminal, inicie o túnel:
                            <code className="block bg-black/40 p-3 rounded border border-white/5 font-mono text-green-400 mt-2">
                                ngrok http 3000
                            </code>
                        </li>
                        <li>
                            Copie o link <code className="text-white">https://....ngrok-free.app</code> e cole no campo "Endereço do Servidor" no topo desta tela.
                        </li>
                    </ol>
                </div>
            )}
